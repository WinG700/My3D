...
void Camera::SetFovX(const float& _FovX)
{
	FovX = _FovX;
}
...
Matrix4x4* Camera::CreatePerPerspectiveMatrix(float N)
{
	...
	PerspectiveMatrix->m[2][0] = 0; PerspectiveMatrix->m[2][1] = 0.0f; PerspectiveMatrix->m[2][2] = 1.0f; PerspectiveMatrix->m[2][3] = -1.0f * N;
